...
{
	set_name("TurboDecoder");
	set_description("Decoder for Turbo Codes");
}
...
#endif
	itpp::bvec out_tmp;
	code.decode(*in_vector_, out_tmp);
	*out_vector_ = to_ivec(out_tmp);
#ifndef NDEBUG
	std::cout << " decoded(" << out_vector_->length() << "): " << *out_vector_ << std::endl;
#endif
}
